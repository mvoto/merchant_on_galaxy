...
class Oracle
  ...
  def self.answer_questions(questions)
    questions.map do |question|
      ...
      credit?(question) ? credit_answer(info) : roman_answer(info)
    end
  end
  ...
  def self.credit_answer(question_info)
  ...
  def self.roman_answer(question_info)
    roman_galaxy_units = question_info.roman_galaxy_units
    return weird_question_message if nil_or_empty_info?(roman_galaxy_units)
    roman = extract_roman(roman_galaxy_units)
    return weird_question_message if nil_or_empty_info?(roman)

    answer_value = NumeralConverter.roman_to_arabic(roman)

    "#{roman_galaxy_units} is #{sprintf("%g", answer_value)}"
  end
